...
class LDAPEnumeratorManager2(multiprocessing.Process):
	def __init__(self, ldap_mgr, agent_in_q, agent_out_q, agents_cnt = None):
		...
		self.test_ctr = 0
		self.agent_tasks = []
		self.agents_cnt = None

	async def arun(self):
		if self.agents_cnt is None:
			self.agents_cnt = multiprocessing.cpu_count()

		agent = LDAPEnumeratorAgent(self.ldap_mgr, self.agent_in_q, self.agent_out_q)
		agent_task = asyncio.create_task(agent.arun())
...
class LDAPEnumeratorManager:
	...
	def check_jobs(self, finished_type):
		...
		print(len(self.finished_enums))
		print(self.enum_types_len)
	...
	def setup(self):
		...
		#agent = LDAPEnumeratorManager2(self.ldam_mgr, self.agent_in_q, self.agent_out_q, self.agent_cnt)
		#agent.daemon = True
		#agent.start()
