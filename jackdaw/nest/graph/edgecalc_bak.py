...
class EdgeCalc:
	...
	def add_edge(self, src_sid, dst_sid, label):
		...
		src_id = self.session.query(JackDawEdgeLookup.id).filter_by(oid = src_sid).filter(JackDawEdgeLookup.ad_id == self.ad_id).first()
		if src_id is None:
			#this should not happen
			t = JackDawEdgeLookup(self.ad_id, src_sid, 'unknown')
			self.session.add(t)
			self.session.commit()
		...
		dst_id = self.session.query(JackDawEdgeLookup.id).filter_by(oid = dst_sid).filter(JackDawEdgeLookup.ad_id == self.ad_id).first()
		if dst_id is None:
			#this should not happen
			t = JackDawEdgeLookup(self.ad_id, dst_sid, 'unknown')
			self.session.add(t)
			self.session.commit()
			self.session.refresh(t)
			dst_id = t.id
		...
		edge = JackDawEdge(self.ad_id, src_id, dst_id, label)
